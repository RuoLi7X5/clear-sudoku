/**
 * 增强手写模板 v2 — 从正确识别的格提取样本
 * 针对混淆数字(3/5/6/8/9)追加额外优质样本
 */
...
async function main() {
  ...
  // ── Find correctly-recognized cells and collect extra samples for target digits ──
  console.log("=== 收集正确识别格的样本 (目标: 3,5,6,8,9) ===\n");
  const targetDigits = new Set([3, 5, 6, 8, 9]);
  const extraSamples: Record<number, number[][][]> = {};
  for (let d = 1; d <= 9; d++) extraSamples[d] = [];
  const maxPerDigit = 3; // only add 3 extra samples per confusing digit
  ...
  for (let pi = 0; pi < 40; pi++) {
    ...
    for (let r = 0; r < 9; r++) {
      for (let c = 0; c < 9; c++) {
        ...
        if (!targetDigits.has(exp)) continue;
        if (extraSamples[exp].length >= maxPerDigit) continue;
        ...
        // Verify: the current template correctly identifies this cell
        const result = matchBigDigit(px, bw, bh, "big");
        if (result.digit === exp && result.confidence > 0.6) {
  ...
  for (const d of targetDigits) {
    ...
    console.log(`  big_${d}.json: +${extraSamples[d].length}样本 (共${tpl.samples.length})`);
  }
    ...
    for (let r = 0; r < 9; r++) {
      for (let c = 0; c < 9; c++) {
        ...
        if (!hasDigit && exp === 0) correct++;
          ...
          else if (hasDigit && got !== exp) { wrong++; const k = `${exp}→${got}`; confusions[k] = (confusions[k]||0)+1; }
          else if (!hasDigit && exp !== 0) missed++;
        }
      }
    }
    totalCorrect += correct; totalWrong += wrong; totalMissed += missed;
    const icon = wrong + missed === 0 ? "✓" : "✗";
    console.log(`  ${icon} #${pn}: ${correct}/81${wrong+missed>0?` (wrong=${wrong} missed=${missed})`:""}`);
  }
