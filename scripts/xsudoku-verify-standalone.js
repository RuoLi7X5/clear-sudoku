...
function main() {
  ...
  console.log("=== Xsudoku 模板独立验证 ===");
  console.log("模板数量:", tc.length, "(", tc.filter(t => t.digit === 1).length, "/digit )\n");

  // ── Test at multiple thresholds ──
  for (const THR of [0.64, 0.65, 0.66]) {
      ...
      for (let r = 0; r < 9; r++) {
        for (let c = 0; c < 9; c++) {
          ...
          if (expected === 0 && got === 0) correct++;
            ...
            allErrors.push({ puzzle: pn, cell, expected, got, conf: bc });
    ...
    console.log("阈值 " + THR.toFixed(2) + ": " + acc + "% | 漏识=" + missed + " 错识=" + wrong + " 误报=" + fp + " 总错=" + allErrors.length);
    ...
    if (allErrors.length > 0) {
      ...
      for (const e of allErrors) {
        const k = e.expected === 0 ? "0→" + e.got : e.got === 0 ? e.expected + "→0" : e.expected + "→" + e.got;
        byType[k] = (byType[k] || 0) + 1;
      }
      const sorted = Object.entries(byType).sort((a, b) => b[1] - a[1]);
      const summary = sorted.slice(0, 5).map(([k, v]) => k + ":" + v).join(", ");
      console.log("  错误类型: " + summary + (sorted.length > 5 ? " ..." : ""));

      // Show first few
      for (const e of allErrors.slice(0, 3)) {
        console.log("    #" + e.puzzle + " " + e.cell + ": " + e.expected + "→" + e.got + " conf=" + e.conf.toFixed(3));
      }
    }
  }
}
