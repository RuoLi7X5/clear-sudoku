/**
 * Xsudoku 模板 v3 — 基于多样本 + 针对性强化 4↔8 歧义
 *
 * 关键认识: 答案文件是完整解，盘面上只有部分格有大数字(已知数+出数)。
 * 不对比空格 → 只对比有数字的格。
 */
...
function selectDiverseSamples(normalizedSamples, N) {
  if (normalizedSamples.length <= N) return normalizedSamples.map((_, i) => i);
  const h = normalizedSamples[0].length, w = normalizedSamples[0][0].length;
  const mean = [];
  for (let y = 0; y < h; y++) { mean[y] = []; for (let x = 0; x < w; x++) { let sum = 0; for (const s of normalizedSamples) sum += s[y][x]; mean[y][x] = sum / normalizedSamples.length; } }
  const scores = normalizedSamples.map((s, i) => { let diff = 0; for (let y = 0; y < h; y++) for (let x = 0; x < w; x++) diff += (s[y][x] - mean[y][x]) ** 2; return { idx: i, dist: diff }; });
  scores.sort((a, b) => a.dist - b.dist);
  const selected = [scores[Math.floor(scores.length / 2)].idx];
  const step = Math.max(1, Math.floor(scores.length / (N - 1)));
  for (let i = 0; i < N - 1 && selected.length < N; i++) { const cand = scores[Math.min(i * step, scores.length - 1)].idx; if (!selected.includes(cand)) selected.push(cand); }
  return selected;
}

// Problem cells that need extra samples
const EXTRA_SAMPLES = [
  { puzzle: 11, row: 3, col: 4, digit: 4 }, // 4 confused with 8
  { puzzle: 11, row: 3, col: 7, digit: 8 }, // 8 confused with 4
];
...
function main() {
  ...
  // ── Extract all samples ──
  console.log("=== 提取样本 (仅对比有数字的格) ===\n");
  const byDigit = {}; for (let d = 1; d <= 9; d++) byDigit[d] = [];
  const extraByDigit = {}; for (let d = 1; d <= 9; d++) extraByDigit[d] = [];
  ...
  for (let pi = 0; pi < 40; pi++) {
    ...
    for (let r = 0; r < 9; r++) {
      for (let c = 0; c < 9; c++) {
        ...
        if (mv < 30) continue; // No visible big digit in this cell

        // Check if this is one of the extra cells
        const isExtra = EXTRA_SAMPLES.some(e => e.puzzle === pn && e.row === r && e.col === c);

        if (isExtra) {
          extraByDigit[solutionDigit].push(pixels);
        } else {
          byDigit[solutionDigit].push(pixels);
        }
      }
    }
  }
  ...
  for (let d = 1; d <= 9; d++) {
    console.log("  数字" + d + ": " + byDigit[d].length + " 样本 + " + extraByDigit[d].length + " 强化样本");
  }

  // ── Build multi-sample templates with extra samples ──
  console.log("\n=== 构建模板 ===\n");
  const N_SAMPLES = 5;
  const templates = {};

  for (let d = 1; d <= 9; d++) {
    let allSamples = [...byDigit[d], ...extraByDigit[d]];
    // Remove duplicates (extra samples might already be in main set)
    // Simple dedup by reference isn't possible with pixel arrays, skip for now

    // Find most common size
    const sz = {};
    for (const s of allSamples) { const k = s.length + "x" + (s[0] ? s[0].length : 0); sz[k] = (sz[k] || 0) + 1; }
    let best = "", bestN = 0;
    for (const [k, n] of Object.entries(sz)) if (n > bestN) { bestN = n; best = k; }
    const [th, tw] = best.split("x").map(Number);

    // Normalize all
    const norm = [];
    for (const s of allSamples) {
      const sh = s.length, sw = s[0] ? s[0].length : 0;
      if (sh === th && sw === tw) norm.push(s);
      else norm.push(scaleTo(s, sw, sh, tw, th));
    }

    // Select N diverse samples, but ensure extra samples are included
    const selected = selectDiverseSamples(norm, Math.max(N_SAMPLES, N_SAMPLES + extraByDigit[d].length));

    // Make sure extra samples are in the selection
    // Extra samples are at the END of norm array
    const extraStartIdx = norm.length - extraByDigit[d].length;
    for (let i = extraStartIdx; i < norm.length; i++) {
      if (!selected.includes(i)) selected.push(i);
    }

    const entries = selected.map(i => {
      const p = norm[i];
      let darkCount = 0;
      for (const row of p) for (const v of row) if (v > 128) darkCount++;
      return { pixels: p, darkCount };
    });

    templates[d] = { w: tw, h: th, samples: entries };
    const extraLabel = extraByDigit[d].length > 0 ? " (含" + extraByDigit[d].length + "强化)" : "";
    console.log("  数字" + d + ": " + tw + "x" + th + ", " + entries.length + " samples" + extraLabel);
  }
  ...
  for (let d = 1; d <= 9; d++) {
    const t = templates[d];
    const mean = [];
    for (let y = 0; y < t.h; y++) { mean[y] = []; for (let x = 0; x < t.w; x++) { let sum = 0; for (const s of t.samples) sum += s.pixels[y][x]; mean[y][x] = Math.round(sum / t.samples.length); } }
    let dc = 0; for (let y = 0; y < t.h; y++) for (let x = 0; x < t.w; x++) if (mean[y][x] > 128) dc++;
    writeFileSync(join(TEMPLATE_DIR, "xsudoku_" + d + ".json"), JSON.stringify({ w: t.w, h: t.h, pixels: mean, darkCount: dc, samples: t.samples }, null, 2));
  ...
  for (let d = 1; d <= 9; d++) {
    ...
    for (const s of t.samples) {
      ...
      tplCache.push({ digit: d, w: t.w, h: t.h, pixels: s.pixels, mean: n > 0 ? sum / n : 0 });
    }
  }
  console.log("模板库: " + tplCache.length + " samples");

  for (const THR of [0.60, 0.62, 0.63, 0.64, 0.65, 0.70]) {
    ...
    for (let pi = 0; pi < 40; pi++) {
      ...
      for (let r = 0; r < 9; r++) {
        for (let c = 0; c < 9; c++) {
          ...
          // Determine ground truth: does this cell HAVE a big digit in the image?
          // We use maxVal as a simple heuristic: if maxVal < 30, no digit (empty or candidates only)
          ...
          if (!hasBigDigit && got === 0) correct++;
            ...
            else if (!hasBigDigit && got !== 0) { fp++; falsePositives.push({puzzle:pn, r, c, got}); }
            else if (hasBigDigit && got !== solutionDigit) wrong++;
          }
        }
      }
    }
    ...
    if (fp) { st.push("误报" + fp); }
    console.log("  阈值" + THR.toFixed(2) + ": " + acc + "% " + (st.length ? st.join(",") : "✓ 完美"));

    if (fp > 0 && fp <= 5) {
      for (const fp of falsePositives) {
        console.log("    FP #" + fp.puzzle + " " + String.fromCharCode(65 + fp.r) + (fp.c + 1) + "→" + fp.got);
      }
    }
  }
  ...
  for (let pi = 0; pi < 40; pi++) {
    ...
    for (let r = 0; r < 9; r++) {
      for (let c = 0; c < 9; c++) {
        ...
        const hasBigDigit = mv >= 30;
        if (hasBigDigit && got !== solutionDigit) {
          const cell = String.fromCharCode(65 + r) + (c + 1);
          console.log("  #" + pn + " " + cell + ": solution=" + solutionDigit + " got=" + got + " conf=" + bc.toFixed(3) + " mv=" + mv);
        }
      }
    }
  }
