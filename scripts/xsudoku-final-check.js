...
const OUTPUT_DIR = join(__dirname, "..", "xsudoku-final");
...
function main() {
  ...
  for(let d=1;d<=9;d++){
    ...
    const entries=raw.samples||[{pixels:raw.pixels}];
  ...
  const THR=0.65;
  console.log(`=== 逐一核实 (阈值=${THR}) ===\n`);

  const errors=[];
  let totalCorrect=0,totalMissed=0,totalWrong=0,totalFP=0;
  ...
  for(let pi=0;pi<40;pi++){
    ...
    for(let r=0;r<9;r++){for(let c=0;c<9;c++){
      ...
      if(maxVal>=30&&bw>=5&&bh>=5){
        ...
        bestConf=(bestScore+1)/2;
        if(bestConf>THR) got=bestDigit;
      }

      if(expected===0&&got===0) totalCorrect++;
      else if(expected===got) totalCorrect++;
      else {
        puzzleErrors.push({cell:`${String.fromCharCode(65+r)}${c+1}`,expected,got,conf:bestConf});
        if(expected!==0&&got===0) totalMissed++;
        else if(expected===0&&got!==0) totalFP++;
        else totalWrong++;
      }
    }}

    if(puzzleErrors.length>0){
      const errs=puzzleErrors.map(e=>`${e.cell}:${e.expected}→${e.got}(conf=${e.conf.toFixed(3)})`).join(", ");
      console.log(`  #${pn}: ${puzzleErrors.length}错 — ${errs}`);
      for(const e of puzzleErrors) errors.push({puzzle:pn,...e});
    } else {
      console.log(`  #${pn}: ✓`);
    }
  }
  ...
  const total=40*81;
  console.log(`\n总计: ${totalCorrect}/${total} (${(totalCorrect/total*100).toFixed(2)}%)`);
  console.log(`漏识=${totalMissed} 错识=${totalWrong} 误报=${totalFP}`);

  writeFileSync(join(OUTPUT_DIR,"final-errors.json"),JSON.stringify(errors,null,2));
}
