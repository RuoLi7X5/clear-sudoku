/**
 * 增强手写模板 — 从错误格提取样本追加到原始模板
 * 保持原始精选样本格式，只追加少量精选补充样本
 */
...
async function main() {
  // ── Known error cells from the full pipeline test ──
  // Format: [puzzle, row, col, expectedDigit]
  const errorCells: Array<[number, number, number, number]> = [
    [1, 0, 6, 8],  // A7: given=3 expected=8
    [2, 5, 8, 8],  // F9: given=6 expected=8
    [4, 5, 7, 8],  // F8: given=6 expected=8
    [5, 1, 8, 9],  // B9: given=5 expected=9
    [12, 0, 8, 9], // A9: given=5 expected=9
    [17, 5, 7, 8], // F8: given=6 expected=8
    [37, 6, 7, 8], // G8: given=6 expected=8
    [39, 0, 8, 3], // A9: given=8 expected=3
    [40, 7, 8, 9], // H9: given=5 expected=9
    // Also add cells where the CORRECT digit is clearly visible
    [1, 0, 3, 5],  // A4: correct 5 (for comparison)
  ];

  // ── Extract pixel samples from error cell positions ──
  console.log("=== 提取错误格样本 ===\n");
  const extraSamples: Record<number, number[][][]> = {};
  for (let d = 1; d <= 9; d++) extraSamples[d] = [];

  for (const [pn, row, col, exp] of errorCells) {
    ...
    const x1 = vL[col], y1 = hL[row], x2 = vL[col + 1], y2 = hL[row + 1];
    ...
    console.log(`  #${pn} ${cell}: exp=${exp} mv=${mv} → 追加到数字${exp}模板`);
    extraSamples[exp].push(px);
  }

  // ── Append samples to existing big templates ──
  console.log("\n=== 更新模板 ===\n");
  const TPL_W = 24, TPL_H = 36;

  for (let d = 1; d <= 9; d++) {
    const tpl = JSON.parse(readFileSync(join(TPL, `big_${d}.json`), "utf-8"));
    const newSamples = extraSamples[d];
    if (newSamples.length === 0) { console.log(`  big_${d}.json: 无新增`); continue; }

    // Normalize new samples to 24×36
    ...
    console.log(`  big_${d}.json: +${newSamples.length}样本 (共${tpl.samples.length})`);
  }

  // ── Re-test with enhanced templates ──
  console.log("\n=== 重新测试 ===\n");
  reloadTemplates();
  ...
  for (let pi = 0; pi < 40; pi++) {
    ...
    for (let r = 0; r < 9; r++) {
      for (let c = 0; c < 9; c++) {
        ...
        for (const row of px) for (const v of row) if (v > mv) mv = v;
        let got = 0;
        const hasDigit = mv >= 30 && bw >= 5 && bh >= 5;
        if (hasDigit) {
        ...
        if (!hasDigit && exp === 0) correct++;
          ...
          puzzleErrors.push(`${cell}:${exp}→${got}`);
    ...
    if (puzzleErrors.length > 0) {
      console.log(`  ✗ #${pn}: wrong=${wrong} missed=${missed} | ${puzzleErrors.slice(0,5).join(" ")}${puzzleErrors.length>5?"...":""}`);
    } else {
      console.log(`  ✓ #${pn}`);
    }
  }
