/**
 * Xsudoku 最终模板 — 97×97 留白（类似 cell 原生尺寸）, 每数字 1 个 mean 样本
 * 像 big 模板一样简洁高效
 */
...
function main() {
  ...
  // ── Extract all samples, normalize to common size per digit ──
  console.log("=== 提取样本 ===\n");
  ...
  for(let pi=0;pi<40;pi++){
    ...
    for(let r=0;r<9;r++){for(let c=0;c<9;c++){
      ...
      if(mv<30)continue;
      byDigit[expected].push({pixels,w:bw,h:bh});
    }}
  }

  for(let d=1;d<=9;d++) console.log(`  数字${d}: ${byDigit[d].length} 样本`);

  // ── Build mean templates (1 per digit, like big templates) ──
  console.log("\n=== 构建模板 (每数字1个mean样本) ===\n");
  const templates={};
  ...
  for(let d=1;d<=9;d++){
    ...
    // Normalize all to common size
    const norm=[];
    for(const s of samples){
      if(s.w===tw&&s.h===th) norm.push(s.pixels);
      else norm.push(scaleTo(s.pixels,s.w,s.h,tw,th));
    }

    // Mean
    const mean=[];
    for(let y=0;y<th;y++){mean[y]=[];for(let x=0;x<tw;x++){let sum=0;for(const np of norm)sum+=np[y][x];mean[y][x]=Math.round(sum/norm.length);}}

    let darkCount=0,sum=0;
    for(let y=0;y<th;y++)for(let x=0;x<tw;x++){if(mean[y][x]>128)darkCount++;sum+=mean[y][x];}

    templates[d]={w:tw,h:th,pixels:mean,darkCount,mean:sum/(tw*th)};
    console.log(`  数字${d}: ${tw}x${th}, darkCount=${darkCount} (${(darkCount/(tw*th)*100).toFixed(1)}%)`);
  }
  ...
  for(let d=1;d<=9;d++){
    ...
    writeFileSync(join(TEMPLATE_DIR,`xsudoku_${d}.json`),JSON.stringify({
      ...
      samples:[{pixels:t.pixels,darkCount:t.darkCount}],
    },null,2));
    console.log(`  xsudoku_${d}.json`);
  }

  // ── Test: 1 sample/digit only (like big templates) ──
  console.log("\n=== 测试 (9 samples total, 1/digit) ===\n");
  const tplCache=[];
  for(let d=1;d<=9;d++){
    const t=templates[d];
    tplCache.push({digit:d,w:t.w,h:t.h,pixels:t.pixels,mean:t.mean});
  }
  console.log(`模板库: ${tplCache.length} samples (9 digits × 1 mean)`);

  for(const thr of [0.60,0.62,0.63,0.64,0.65,0.66,0.67,0.68,0.70,0.72]){
