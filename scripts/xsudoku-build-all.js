...
function main() {
  ...
  // ── Step 1: Extract all samples from all puzzles ──
  console.log("=== 提取所有样本 ===\n");
  ...
  for (let pi=0; pi<40; pi++) {
    ...
    for(let r=0;r<9;r++){
      for(let c=0;c<9;c++){
        ...
        allSamples.push({digit:expected,pixels,w,h,puzzle:pn,cell:`${String.fromCharCode(65+r)}${c+1}`});
      }
    }
  }
  ...
  // ── Step 2: Pick target size per digit (most common), normalize all, average ──
  console.log("\n=== 构建平均模板 ===\n");
  const templates={};
  ...
  for(let d=1;d<=9;d++){
    ...
    // Normalize ALL samples to this size
    const normalized=[];
    ...
    for(let y=0;y<th;y++){
      avg[y]=[];
      for(let x=0;x<tw;x++){
        let sum=0; for(const np of normalized) sum+=np[y][x];
        avg[y][x]=Math.round(sum/normalized.length);
      }
    }
    ...
    // Also pick top-3 samples closest to average (as individual samples)
    const scored=normalized.map((np,i)=>({idx:i,score:ncc(np,{w:tw,h:th,pixels:avg,mean})}));
    scored.sort((a,b)=>b.score-a.score);
    const topSamples=scored.slice(0,3).map(s=>({pixels:normalized[s.idx],darkCount:0}));

    templates[d]={w:tw,h:th,pixels:avg,darkCount,mean,samples:topSamples};
    console.log(`  数字${d}: ${tw}x${th}, ${samples.length}样本→平均, 精选${topSamples.length}样本`);
  }
  ...
  for(let d=1;d<=9;d++) if(templates[d]){
    // Add mean template
    tplCache.push({digit:d,w:templates[d].w,h:templates[d].h,pixels:templates[d].pixels,mean:templates[d].mean});
  ...
  console.log(`模板库: ${tplCache.length} 个 (${9} mean + ${9*3} top samples)`);
  ...
  for(const thr of [0.55,0.60,0.65,0.70]){
      ...
      for(let r=0;r<9;r++){for(let c=0;c<9;c++){
        ...
        if(maxVal>=30&&bw>=5&&bh>=5){
          ...
          for(const tp of tplCache){
            const sc=ncc(pixels,tp);if(sc>bestScore){bestScore=sc;bestDigit=tp.digit;}
          }
    ...
    console.log(`  阈值${thr.toFixed(2)}: ${acc}% 漏识=${missed} 错识=${wrong} 误报=${fp}`);
  }
