/**
 * Xsudoku 多样本模板 — 每数字选 5 个代表性样本
 * 选择策略: k-means 风格的聚类选择，覆盖字体变化
 */
...
/**
 * Diversified sample selection: pick N samples that are maximally different from each other
 * to cover the natural variation in the font.
 */
function selectDiverseSamples(normalizedSamples, N) {
  ...
  for (let y = 0; y < h; y++) {
    ...
    for (let x = 0; x < w; x++) {
      let sum = 0;
      for (const s of normalizedSamples) sum += s[y][x];
      mean[y][x] = sum / normalizedSamples.length;
    }
  }

  // Compute each sample's distance to mean
  const scores = normalizedSamples.map((s, i) => {
    let diff = 0;
    for (let y = 0; y < h; y++)
      for (let x = 0; x < w; x++)
        diff += (s[y][x] - mean[y][x]) ** 2;
    return { idx: i, dist: diff };
  });
  scores.sort((a, b) => a.dist - b.dist);

  // Pick: the median sample, then N-1 samples spread across the distribution
  const selected = [];
  // Always include the median (closest to mean)
  selected.push(scores[Math.floor(scores.length / 2)].idx);
  // Then add evenly-spaced samples
  const step = Math.max(1, Math.floor(scores.length / (N - 1)));
  for (let i = 0; i < N - 1 && selected.length < N; i++) {
    const idx = Math.min(i * step, scores.length - 1);
    const cand = scores[idx].idx;
    if (!selected.includes(cand)) selected.push(cand);
  }
  // Fill with random if needed
  while (selected.length < N) {
    const rand = Math.floor(Math.random() * normalizedSamples.length);
...
function main() {
  ...
  for (let d = 1; d <= 9; d++) {
    ...
    const sz = {};
    for (const s of samples) { const k = s.length + "x" + (s[0] ? s[0].length : 0); sz[k] = (sz[k] || 0) + 1; }
    let best = "", bestN = 0;
    ...
    // Select N diverse samples
    const selected = selectDiverseSamples(norm, N_SAMPLES);
    ...
    console.log("  数字" + d + ": " + tw + "x" + th + ", " + entries.length + " samples (from " + samples.length + " total)");
  }
  ...
  for (let d = 1; d <= 9; d++) {
    const t = templates[d];
    // Also compute mean for the main "pixels" field
    const mean = [];
    for (let y = 0; y < t.h; y++) {
      mean[y] = [];
      for (let x = 0; x < t.w; x++) {
        let sum = 0;
        for (const s of t.samples) sum += s.pixels[y][x];
        mean[y][x] = Math.round(sum / t.samples.length);
      }
    }
    let dc = 0;
    for (let y = 0; y < t.h; y++) for (let x = 0; x < t.w; x++) if (mean[y][x] > 128) dc++;

    writeFileSync(join(TEMPLATE_DIR, "xsudoku_" + d + ".json"), JSON.stringify({
      w: t.w, h: t.h,
      pixels: mean,
      darkCount: dc,
      samples: t.samples,
  ...
  console.log("模板库: " + tplCache.length + " samples (9 digits × " + N_SAMPLES + ")");
  ...
  for (const THR of [0.60, 0.62, 0.63, 0.64, 0.65, 0.66, 0.67, 0.68, 0.70]) {
    ...
    const acc = correct / (40 * 81) * 100;
    let st = [];
    if (missed) st.push("漏识" + missed);
    if (wrong) st.push("错识" + wrong);
    if (fp) st.push("误报" + fp);
    console.log("  阈值" + THR.toFixed(2) + ": " + acc.toFixed(2) + "% " + (st.length ? st.join(",") : "✓ 完美"));
  }

  // ── Show remaining errors at best threshold ──
  const BEST_THR = 0.64;
  console.log("\n=== 阈值 " + BEST_THR + " 错误详情 ===\n");
  for (let pi = 0; pi < 40; pi++) {
    const pn = pi + 1, answer = A[pi];
    ...
    for (let r = 0; r < 9; r++) {
      for (let c = 0; c < 9; c++) {
        ...
        if (mv >= 30 && bw >= 5 && bh >= 5) {
          ...
          bc = (bs + 1) / 2;
          if (bc > BEST_THR) got = bd;
        }
        if (expected !== got) {
          const cell = String.fromCharCode(65 + r) + (c + 1);
          console.log("  #" + pn + " " + cell + ": " + expected + "→" + got + " conf=" + bc.toFixed(3));
        }
      }
    }
  }
