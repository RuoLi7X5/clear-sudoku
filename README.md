...
- **73 套模板**：手写+数字+Xsudoku+水印+68 系统字体，自动发现加载
...
├── template-match.ts   # NCC 模板匹配（73套模板自动发现、手写优先）
...
| 候选 | small | 小数字备份 |
| 系统字体 | digital | 自渲染闭环 |
| Xsudoku | xsudoku | Xsudoku 专用 |
| 水印 | wm | 16px 红色数字 |
| 68 系统字体 | simhei, arial... | 自动发现 |

训练新字体：`npx ts-node scripts/gen-font-templates.ts <字体路径> <字体名> <前缀>`
